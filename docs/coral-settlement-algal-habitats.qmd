...
algae_broad <- read_xlsx(path = '../data/primary/CH3-Algal-community-time-0.xlsx', sheet = 1) |>
  select(!Date)
algae_local <- read_xlsx(path = '../data/primary/CH3-Algal-community-time-0.xlsx', sheet = 2) |>
  select(!Date)
...
1.  Change % cover and perimeter of "+" into 1 (rare taxa)
2.  Turn into integer
...
# Model 2 - Sargassum height at broad scale
...
form <- bf(Total ~ Treatment + H_mean_broad, family = poisson(link = 'log')) 
form |> get_prior(data = recruit)
